...
export default function EmailPolicy() {
  ...
  const data = [
    ...
    {
      checked: isChecked3,
      handle: handleCheckboxChange3,
      text: "Acepto que me envíen promociones y eventos a mi correo electrónico",
    },
  ];
  ...
  const handleRegister = async () => {
    ...
    if (!dataUsuario) {
      setError(
        "No se encontraron datos de usuario. Por favor, regístrate nuevamente."
      );
      setIsLoading(false);
      return;
    }
    ...
    try {
      const response = await fetch(
        "http://localhost:5001/api/signup/phoneNumber",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="body-policy">
      <a className="back-arrow-policy" href="/signup/email">
        <GoBackArrow />
      </a>
              ...
              <label className="label-policy">
                ...
                {policy.link && (
                  <>
                    ...
                    <a href={policy.href} className="label-policy-link">
                      {policy.link}
                    </a>
                  </>
                )}
